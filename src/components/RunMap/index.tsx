...
import type { Coordinate, IViewState } from '@/utils/geoUtils';
...
const RunMap = ({
  ...
  useEffect(() => {
    if (mapRef.current) {
      ...
      return () => {
        ...
        if (restoreStyleTimer) clearTimeout(restoreStyleTimer);
      };
    }
  }, [mapStyle, lights, switchLayerVisibility]);
  ...
  useEffect(() => {
    if (isBigMap && IS_CHINESE && !mapGeoData && !isLoadingMapDataRef.current) {
      ...
      geoJsonForMap()
        ...
        .catch(() => {
          // tile errors reported separately
        })
        .finally(() => {
  ...
  const startRouteAnimation = useCallback(() => {
    ...
    if (routeAnimatorRef.current) routeAnimatorRef.current.stop();
    ...
    >
      ...
      <Source id="data" type="geojson" data={combinedGeoData}>
        <Layer
          id="province"
          type="fill"
          paint={{ 'fill-color': PROVINCE_FILL_COLOR }}
            ...
            features: [
              {
                ...
                geometry: { type: 'LineString', coordinates: animatedPoints },
              },
            ],
          }}
        >
